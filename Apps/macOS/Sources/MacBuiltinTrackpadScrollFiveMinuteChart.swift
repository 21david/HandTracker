...
private enum LiveBuiltinTrackpadScrollChart {
...
struct MacBuiltinTrackpadScrollFiveMinuteChart: View {
    ...
    var body: some View {
        let _ = livePulse.builtinTrackpadScroll
        let liveRevision = MacChartEquatableBucket.builtinTrackpadScrollRevision(store)
        TimelineView(.periodic(from: .now, by: 30)) { timeline in
            MacBuiltinTrackpadScrollFiveMinuteChartRender(
                store: store,
...
private struct MacBuiltinTrackpadScrollFiveMinuteChartRender: View, Equatable {
    ...
    private var cap: Double { resolution.scaledCap(fiveMinuteCap: LiveBuiltinTrackpadScrollChart.fiveMinuteCap) }
    private var excess: Double { resolution.scaledExcess(fiveMinuteExcess: LiveBuiltinTrackpadScrollChart.fiveMinuteExcess) }

    var body: some View {
        let slots = store.builtinTrackpadScrollByFiveMinuteSlotsTrailing(
        ...
        Chart {
            RuleMark(y: .value("Baseline", 0.0))
                .foregroundStyle(LiveBuiltinTrackpadScrollChart.axisLineColor)
                .lineStyle(StrokeStyle(lineWidth: 1))
            ForEach(Array(slots.enumerated().compactMap { index, slot -> Plotted? in
                guard slot.scrollPixels > 0 else { return nil }
                return Plotted(index: index, pixels: slot.scrollPixels)
            })) { plotted in
        ...
        .chartYAxisLabel(position: .leading) {
            MacFiveMinuteChartLeadingCaption.rotated180Degrees("Scroll travel", deviceNote: "macbook trackpad")
        }
        .frame(height: 200)
        .padding(.top, 20)
    }
    ...
    private func tickDate(idx: Int, slots: [BuiltinTrackpadScrollFiveMinuteSlot]) -> Date? {
